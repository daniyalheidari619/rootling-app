...
export default function PostScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (taskId) {
        navigation.navigate('Payment', { taskId });
      } else {
...
const styles = StyleSheet.create({
  ...
  modeToggle: { flexDirection: 'row', backgroundColor: '#F3F4F6', borderRadius: 12, padding: 4, marginHorizontal: 16, marginBottom: 16, gap: 4 },
  modeBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 10 },
  modeBtnActive: { backgroundColor: '#1FB6AE' },
  modeBtnLocked: { backgroundColor: '#F9FAFB' },
  modeBtnText: { fontSize: 13, fontWeight: '600', color: '#6B7280' },
  modeBtnTextActive: { color: '#fff' },
  modeBtnTextLocked: { color: '#9CA3AF' },
  premiumBadge: { fontSize: 10, color: '#7C3AED' },
